                        ...
                        <TransitionChild as="template" enter="transition ease-in-out duration-300 transform" enter-from="translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-300 transform" leave-from="translate-x-0" leave-to="translate-x-full">
                            <DialogPanel class="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-12 shadow-xl">
                                <div class="flex items-center justify-between px-4">
                                    ...
                                    <button class="-mr-2 flex h-10 w-10 items-center justify-center rounded-md bg-white p-2 text-gray-400" type="button" @click="mobileFiltersOpen = false">
                                        ...
                                        <DisclosurePanel class="pt-6">
                                            <div class="space-y-6">
                                                <div v-for="(option, optionIdx) in section.options" :key="option.value" class="flex items-center">
                                                    <input :id="`filter-mobile-${section.id}-${optionIdx}`" :checked="option.checked" :name="`${section.id}[]`" :value="option.value" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" type="checkbox" />
                ...
                <div class="flex items-baseline justify-between border-b border-gray-200 pt-24 pb-6">
                    ...
                    <div class="flex items-center">
                        <Menu as="div" class="relative inline-block text-left">
                            <div>
                                <MenuButton class="group inline-flex justify-center text-sm font-medium text-gray-700 hover:text-gray-900">
                                    Sort
                                    <ChevronDownIcon aria-hidden="true" class="-mr-1 ml-1 h-5 w-5 flex-shrink-0 text-gray-400 group-hover:text-gray-500" />
                                </MenuButton>
                            </div>
                                ...
                                <MenuItems class="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none">
                                    <div class="py-1">
                                        <MenuItem v-for="option in sortOptions" :key="option.name" v-slot="{ active }">
                                            <a :class="[option.current ? 'font-medium text-gray-900' : 'text-gray-500', active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm']" :href="option.href">{{ option.name }}</a>
                                        </MenuItem>
                                    </div>
                                </MenuItems>
                            </transition>
                        ...
                        <button class="-m-2 ml-5 p-2 text-gray-400 hover:text-gray-500 sm:ml-7" type="button">
                            ...
                            <Squares2X2Icon aria-hidden="true" class="h-5 w-5" />
                        </button>
                        <button class="-m-2 ml-4 p-2 text-gray-400 hover:text-gray-500 sm:ml-6 lg:hidden" type="button" @click="mobileFiltersOpen = true">
                ...
                <section aria-labelledby="products-heading" class="pt-6 pb-24">
                    ...
                    <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                        ...
                        <form class="hidden lg:block">
                            <h3 class="sr-only">Categories</h3>
                            <ul class="space-y-4 border-b border-gray-200 pb-6 text-sm font-medium text-gray-900" role="list">
                                <li v-for="category in subCategories" :key="category.name">
                            ...
                            <Disclosure v-for="section in filters" :key="section.id" v-slot="{ open }" as="div" class="border-b border-gray-200 py-6">
                                ...
                                <DisclosurePanel class="pt-6">
                                    <div class="space-y-4">
                                        <div v-for="(option, optionIdx) in section.options" :key="option.value" class="flex items-center">
                                            <input :id="`filter-${section.id}-${optionIdx}`" :checked="option.checked" :name="`${section.id}[]`" :value="option.value" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" type="checkbox" />
